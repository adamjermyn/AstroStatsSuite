...
fitNames = ["Medians","Ours","Kriging","BoostTree","MARS"]
fitFuncs = [medianFit,ourFit, krigingFit, boostTreeFit,marsFit]
fitArgs = [(5,),tuple(),tuple(),tuple(),tuple()]
giveErrors = [False, False, False, False, False]

# Make the starting point depend on the fit method

numTrials = [400,400,400,400,400]
biases = []

for i in range(len(fitFuncs)):
	biases.append([])
	for k in range(200,340,20):
		print(i,k)
...
print(biases)
fi = open('../Benchmark Output/findNtransitMedium.out','wb')
import pickle
